...
class UpdateState extends MusicBeatState
{
	...
	override function create()
	{
		...
		text.setFormat("nintendo_NTLG-DB_001", 18, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
		add(text);
		text.screenCenter(X);
		text.y = 290;
		...
		progressText.setFormat("nintendo_NTLG-DB_001", 16, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
		add(progressText);

		download_info = new FlxText(progressBar.x + progBar_bg.width, progressBar.y + progBar_bg.height, 0, "0B / 0B", 16);
		download_info.setFormat("nintendo_NTLG-DB_001", 16, FlxColor.WHITE, CENTER, OUTLINE, FlxColor.BLACK);
		add(download_info);
